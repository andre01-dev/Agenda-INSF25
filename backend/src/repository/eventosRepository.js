...
export async function inserirEvento(evento) {
    ...
    let resposta = await con.query(comando, [evento.titulo, evento.descricao, evento.data_do_evento])
    let info = resposta[0];
...
export async function alterarEvento(id, evento) {
    ...
    let resposta = await con.query(comando, [evento.titulo, evento.descricao, evento.data_do_evento, id])
    let info = resposta[0];
